...
const skillRegistry = createAgentSkillRegistry({
  skills: [
    ...
    },
    {
      ...
      summary: 'Explains scoped node.add in an AI Group sandbox.',
      triggers: {
...
test('orchestrator emits semantic commands for joined and text events through the authority bus', async () => {
  ...
  const authority = {
    ...
    dispatch: (input: { actor: { id: string; role: string }; command: Record<string, unknown>; dryRun?: boolean }) => {
  ...
  const chatClient = {
    ...
    completeJson: async (input: { messages: Array<{ content: string }> }) => {
      ...
      if (prompt.includes('"type":"client.joined"')) {
        return {
          raw: { ok: true },
          content: '{"id":"turn:join","summary":"welcome","commands":[{"type":"node.add","scopeGroupId":"group:agent","node":{"id":"agent:greeting","type":"display-breathing","position":{"x":20,"y":0},"config":{"intensity":0.5,"breathRate":1.2},"inputValues":{},"outputValues":{}}}]}',
          parsed: {
            id: 'turn:join',
            summary: 'welcome',
            commands: [
      ...
      return {
        raw: { ok: true },
        content: '{"id":"turn:text","summary":"adjust","commands":[{"type":"node.params.update","scopeGroupId":"group:agent","nodeId":"display:breath","params":{"intensity":0.7}}]}',
        parsed: {
          id: 'turn:text',
          summary: 'adjust',
          commands: [
  ...
  assert.equal(joined.turn.plan?.commands.length, 1);
  assert.equal(dispatches.length, 2);
  assert.equal(dispatches[0].command.type, 'node.add');
  assert.equal(dispatches[0].dryRun, true);
  assert.equal(dispatches[0].command.scopeGroupId, 'group:agent');
  ...
  assert.equal(text.turn.plan?.commands[0]?.type, 'node.params.update');
  ...
  assert.equal(dispatches[3].command.scopeGroupId, 'group:agent');
  assert.equal(prompts.some((prompt) => prompt.includes('Display Breathing Node')), true);
  assert.equal(prompts.some((prompt) => prompt.includes('client.joined')), true);
  assert.equal(prompts.some((prompt) => prompt.includes('client.text.final')), true);
});
