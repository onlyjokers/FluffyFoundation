...
type FixtureCommandAudit = {
  rollbackToken?: string;
  policy?: { allowed: boolean; reason?: string };
  lifecycle?: string[];
};

type FixtureRollbackResult = {
  ok: boolean;
  message?: string;
  recovery?: unknown;
...
const actor: AiSemanticActor = { id: 'ai:ff18-wp4', role: 'ai' };
const createdAt = () => new Date(0).toISOString();

const displayDefinition = (type = 'display-breathing'): FixtureDefinition => ({
  type,
...
const flashlightDefinition = (): FixtureDefinition => ({
...
const createFixtureBus = (initialSnapshot: FixtureSnapshot): FixtureBus => {
