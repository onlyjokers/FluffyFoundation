...
export class ManagerSDK {
    ...
    if (item.action === 'modulateSoundUpdate') {
      ...
      if (idx >= 0) {
        ...
        existing.items[idx] = {
          ...
          payload: mergeControlPayload(prev.payload, item.payload) as BaseControlPayload,
   ...
   */
  sendControlToSelected(action: ControlAction, payload: ControlPayload, executeAt?: number): void {
    if (this.state.selectedClientIds.length === 0) return;
    this.sendControl(targetClients(this.state.selectedClientIds), action, payload, executeAt);
  }

  /**
   * Send control to all clients
   */
  sendControlToAll(action: ControlAction, payload: ControlPayload, executeAt?: number): void {
   ...
   */
  screenColor(
    payload: { color: string; opacity?: number } | ScreenColorPayload,
    toAll = false,
    executeAt?: number
  ): void {
    const normalized: ScreenColorPayload =
      'mode' in payload || Array.isArray((payload as ScreenColorPayload).cycleColors)
        ? { mode: 'solid', opacity: 1, ...(payload as ScreenColorPayload) }
        : {
            color: (payload as { color: string }).color,
            opacity: (payload as { opacity?: number }).opacity,
            mode: 'solid',
          };
